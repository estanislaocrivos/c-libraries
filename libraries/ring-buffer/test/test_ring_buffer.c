...
void test_use_not_initialized(void)
{
    ...
    bool full = false;
    TEST_ASSERT_EQUAL(-EPERM, ring_buffer_is_full(&rb, &full));
...
void test_available(void)
{
    ...
    TEST_ASSERT_EQUAL(0, ring_buffer_push(&rb, test_data, sizeof(test_data)));

    size_t available_space = 0;
    ring_buffer_available(&rb, &available_space);
...
void test_pop_partial(void)
{
    ...
    TEST_ASSERT_EQUAL(0, ring_buffer_pop(&rb, pop_buffer, 3));
...
void test_push_pop_alternated(void)
{
    ...
    TEST_ASSERT_EQUAL(0, ring_buffer_push(&rb, data1, 5));

    uint8_t pop1[3] = {0};
    TEST_ASSERT_EQUAL(0, ring_buffer_pop(&rb, pop1, 3));
    TEST_ASSERT_EQUAL_UINT8_ARRAY(data1, pop1, 3);

    static const uint8_t data2[] = {60, 70};
    TEST_ASSERT_EQUAL(0, ring_buffer_push(&rb, data2, 2));

    uint8_t pop2[4] = {0};
    TEST_ASSERT_EQUAL(0, ring_buffer_pop(&rb, pop2, 4));
