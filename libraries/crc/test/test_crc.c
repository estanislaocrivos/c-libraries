...
// Used https://crccalc.com/?crc=123456789&method=&datatype=ascii&outtype=hex to
...
{
    const struct crc crc8
        = {.crc8_final_xor_value = 0xFF,
...
void test_crc8_proprietary(void)
{
    ...
    int8_t status = crc8_calculate(&crc8, data_1, sizeof(data_1), &result);
    TEST_ASSERT_EQUAL_INT8(0, status);
    TEST_ASSERT_EQUAL_UINT8(0x33, result);
}

/* ========================================================================== */

void test_crc8_null_pointer_errors(void)
{
    const struct crc crc8
        = {.crc8_final_xor_value = 0x00,
           .crc8_initial_value   = 0x00,
    ...
    TEST_ASSERT_EQUAL_INT8(
        -EFAULT, crc8_calculate(NULL, data, sizeof(data), &result));
    TEST_ASSERT_EQUAL_INT8(
        -EFAULT, crc8_calculate(&crc8, NULL, sizeof(data), &result));
    TEST_ASSERT_EQUAL_INT8(
        -EFAULT, crc8_calculate(&crc8, data, sizeof(data), NULL));
}

/* ========================================================================== */

void test_crc8_invalid_length(void)
{
    const struct crc crc8
        = {.crc8_final_xor_value = 0x00,
           .crc8_initial_value   = 0x00,
    ...
    TEST_ASSERT_EQUAL_INT8(-EINVAL, crc8_calculate(&crc8, data, 0, &result));
}
